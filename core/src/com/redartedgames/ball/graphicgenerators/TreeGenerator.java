...
public class TreeGenerator {
	...
	public static Texture generate(int ii) {
		...
		for(Vector2 v: tab) {
			pixmap2.fillRectangle((int)v.x, height-(int)v.y, 6*k, 6*k);
			pixmap2.fillRectangle((int)v.x-6*k, height-(int)v.y-6*k, 6*k, 6*k);
			pixmap2.fillRectangle((int)v.x-3*k, height-(int)v.y, 3*k, 3*k);
			pixmap2.fillRectangle((int)v.x, height-(int)v.y-3*k, 3*k, 3*k);
			pixmap2.drawRectangle((int)v.x- 1*k, height-(int)v.y-1*k, 2*k, 2*k);
			
		}
		
		
		PixmapIO.writePNG(Gdx.files.external("/trees/tree"+ ii +".png"), pixmap);
		PixmapIO.writePNG(Gdx.files.external("/trees/leaves"+ ii +".png"), pixmap2);
	...
	public static void mutate(int i, int n) {
		doGory = (mindoGory * i + (n-i) * maxdoGory)/n;
		thickness2 = (minthickness2 * i + (n-i) * maxthickness2)/n;
		thicknessRate = (minthicknessRate * i + (n-i) * maxthicknessRate)/n;
		radiusLengthRate = (minradiusLengthRate * i + (n-i) * maxradiusLengthRate)/n;
		radiusSmoothRate = (minradiusSmoothRate * i + (n-i) * maxradiusSmoothRate)/n;
		radiusScale = (minradiusScale * i + (n-i) * maxradiusScale)/n;
		alfaDelta = (minalfaDelta * i + (n-i) * maxalfaDelta)/n;
		alfaRate = (minalfaRate * i + (n-i) * maxalfaRate)/n;
		wazkosc = (minwazkosc * i + (n-i) * maxwazkosc)/n;
		losowosc2 = (minlosowosc2 * i + (n-i) * maxlosowosc2)/n;
		losowoscDelta = (minlosowoscDelta * i + (n-i) * maxlosowoscDelta)/n;
	}
