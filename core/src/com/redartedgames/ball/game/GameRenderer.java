...
public class GameRenderer extends ScreenRenderer{
	...
	int treeNumb = 4;
	int treePosX = 500;
	SpriteObject[] trees = new SpriteObject[20];
	SpriteObject[] leaves = new SpriteObject[20];
	...
	public void loadNextLvl() {
		...
		if ((int)(1250/12f - currentLevelId*levelDelta + treePosX/12f) < 0) {
	...
	private void initializeAdvancedGraphic() {
		...
		for (int i = 20-treesI; i < 20; i++) {
			trees[i-20+treesI] = new SpriteObject(200, 300, null, 0);
			trees[i-20+treesI].addTexture("trees/tree" + (i+50) + ".png");
			leaves[i-20+treesI] = new SpriteObject(200, 300, null, 0);
			leaves[i-20+treesI].addTexture("trees/leaves" + (i+50) + ".png");
		}
	...
	public void renderAdvancedGraphic() {
		...
		float treeHeight = 0;
		treeHeight = h1[(int)(1250/12f - currentLevelId*levelDelta + treePosX/12f)] -190;
		Color groundColor = new Color(c1.r/q, c1.g/q, c1.b/q, 1);
		// batch.draw(treeTex, -dxfl*12 - currentLevelId*levelDelta*12 + treePosX, treeHeight);
		for (int i = 0; i < treeNumb; i++) {
			treeHeight = h1[(int)(1250/12f - currentLevelId*levelDelta + treePosX/12f + i * 50/12f)] -220;
			treeHeight *= 1f;
			trees[i].setColor(groundColor);
			trees[i].position.set(i * 50 + -dxfl*12 - currentLevelId*levelDelta*12 + treePosX + 750, treeHeight + 750);
			trees[i].render(batch, 1);
			leaves[i].setColor(new Color(0.2f, 0.8f, 1, 0.07f));
			leaves[i].position.set(i * 50 + -dxfl*12 - currentLevelId*levelDelta*12 + treePosX + 750, treeHeight + 750);
			leaves[i].render(batch, 1);
		}
		
		for (int i = 1; i < treeNumb; i++) {
			treeHeight = h1[Math.max(0, (int)(1250/12f - currentLevelId*levelDelta + treePosX/12f - i * 50/12f))] -220;
			treeHeight *= 1f;
			trees[i].setColor(groundColor);
			trees[i].position.set(-i * 50 + -dxfl*12 - currentLevelId*levelDelta*12 + treePosX + 750, treeHeight + 750);
			trees[i].render(batch, 1);
