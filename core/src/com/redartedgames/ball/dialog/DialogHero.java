...
public class DialogHero extends ReversableObject{
	...
	public DialogHero(float x, float y, int id, GameObject parent, int dialogHeroType, String myText, int width, int height) {
		super(x, y, parent, id);
		this.dialogHeroType = dialogHeroType;
		dialogHeroSprite = new DialogHeroSprite(x, y, this, 0, dialogHeroType);

		textBg = new DialogBackground(x-25 + 25, y + 50 + height, 0, this, width + 50, height);
		text = new ConversationIntro(x + 15-width/2, y + 100 + 7 + height - 50, 0, this, myText);
		window = new DialogConversationWindow(textBg.getPosition().x, textBg.getPosition().y + textBg.height/2, 0, this);
		gameObjects.add(dialogHeroSprite);
		gameObjects.add(window);
		hideText();
	...
	public void showText() {
		textBg.isOn = true; text.isOn = true; isTextShown = true;
		dialogHeroSprite.activate();
	}
	public void hideText() {
		textBg.isOn = false; text.isOn = false; isTextShown = false;
		dialogHeroSprite.deactivate();
	}
